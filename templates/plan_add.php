...
<dl>
 ...
 <dt>车辆要求</dt>
    ...
    <?php foreach($old_tours as $i=>$tour):?>
        <li class="ui-state-default tour" id="item_<?php echo $i;?>"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span><div class="tour_name"><?php echo $tour['name'];?></div><input type="hidden" id="item_<?php echo $i;?>_tour_id" name="plan[item_tour_id][<?php echo $i;?>]" value="<?php echo $tour['id'];?>" />
    ...
    <script id="tourTemplate" type="text/x-jquery-tmpl">
        <li class="ui-state-default tour" id="item_${i}"><span class="ui-icon ui-icon-arrowthick-2-n-s"></span><div class="tour_name">${name}</div><input type="hidden" id="item_${i}_tour_id" name="plan[item_tour_id][${i}]" value="${id}" />
...
$(document).ready(function(){
    ...
    $('input.btn_add_tour').live('click', function(){
        ...
        var li =
        $("#tourTemplate").tmpl({i: idx, id: tour.id, name: tour.name});
