...
<table class="table table-bordered table-striped table-list">
    ...
    <tr>
    <td>
    <label class="checkbox inline"><input type="checkbox" name="ids[]" value="<?php echo $schedule_template_cate['id'];?>" /><?php echo $schedule_template_cate['id'];?></label></td>

        <td><?php echo $schedule_template_cate['name'];?></td>

        <td><?php echo $schedule_template_cate['description'];?></td>
            <td>
                <?php if(checkPrivilege('schedule_template_cate', 'view')):?>
