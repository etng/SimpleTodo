...
<dd>
  <ul id="schedules" class="unstyled sortable">
    ...
    <li class="ui-state-default tour" id="item_<?php echo $i;?>"> <span class="sortable-handle"> <span class="ui-icon ui-icon-arrowthick-2-n-s"></span> D<?php echo $i+1;?>
      <input type="hidden" id="item_<?php echo $i;?>_num" name="plan[item_the_num][<?php echo $i;?>]" value="<?php echo $i+1;?>" size="3"/>
      (
      <input type="hidden" id="item_<?php echo $i;?>_date" name="plan[item_the_date][<?php echo $i;?>]" value="<?php echo date("Y-m-d", strtotime(sprintf("+%d days", $i), $start_date))?>" />
      <?php echo date("Y-m-d", strtotime(sprintf("+%d days", $i), $start_date))?>)
      <input type="hidden" id="item_<?php echo $i;?>_tourist_cnt" name="plan[item_tourist_cnt][<?php echo $i;?>]" value="<?php echo $tourist_cnt;?>" size="3"/>
      <?php echo $tourist_cnt;?>人 </span>
      <div class="tour_info"> <?php echo $tour['name'];?>(<span class="tour_distance"><?php echo $tour['distance'];?>公里</span> 住<span class="tour_destination"><?php echo $tour['destination'];?></span>)
        <input type="hidden" id="item_<?php echo $i;?>_tour_id" name="plan[item_tour_id][<?php echo $i;?>]" value="<?php echo $tour['id'];?>" />
      </div>
      <span class="controls">
      <label class="checkbox inline">
        <input type="hidden" name="plan[item_need_room][<?php echo $i;?>]" value="0" />
        <input type="checkbox" id="item_<?php echo $i;?>_need_room" name="plan[item_need_room][<?php echo $i;?>]" value="1" checked="true" />
        是否安排住宿</label>
      <label class="checkbox inline">
        <input type="hidden" name="plan[item_need_car][<?php echo $i;?>]" value="0" />
        <input type="checkbox" id="item_<?php echo $i;?>_need_car" name="plan[item_need_car][<?php echo $i;?>]" value="1" checked="true" />
        是否安排车辆</label>
  ...
  <div id="tour_selector" style="display:none;">
    ...
    <ul>
      ...
      <li class="tour"><?php echo $tour['name'];?>(<?php echo $tour['price'];?>/<?php echo $tour['market_price'];?>)
        <input data-tour_id=<?php echo $i;?> type="button" value="添加" class="btn btn_add_tour" />
      </li>
      <?php endforeach;?>
    </ul>
  </div>
