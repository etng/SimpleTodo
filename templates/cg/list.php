...
[?php if(checkPrivilege('<?php echo $table['name'];?>', '<?php echo $act; ?>')):?]
    <a href="<?php echo $table['name'];?>.php?act=<?php echo $act; ?>" class="btn <?php echo $act_config['class']; ?>"><?php echo $act_config['label']; ?></a>
[?php endif;?]
<?php endforeach;?>
 </p>
...
<table class="table table-bordered table-striped table-list">
    ...
    <tr>
    <td>
    <label class="checkbox inline"><input type="checkbox" name="ids[]" value="[?php echo $<?php echo $table['name'];?>['id'];?]" />[?php echo $<?php echo $table['name'];?>['id'];?]</label></td>
    <?php foreach($table['fields'] as $field):?>
        <?php if($field->primary && $field->name=='id')continue;?>
            <?php if($field->data_type=='text')continue;?>
    ...
    <?php endforeach;?>
        <td>
        <?php foreach($actions['object'] as $act=>$act_config):?>
        [?php if(checkPrivilege('<?php echo $table['name'];?>', '<?php echo $act; ?>')):?]
            <a href="<?php echo $table['name'];?>.php?act=<?php echo $act; ?>&id=[?php echo $<?php echo $table['name'];?>['id'];?]" class="btn <?php echo $act_config['class']; ?>"><?php echo $act_config['label']; ?></a>
        [?php endif;?]
        <?php endforeach;?>
        </td>
    </tr>
    ...
    <tr>
        <td colspan="100">
        暂时还没有找到相关的<?php echo $table['comment'];?>
        </td>
    </tr>
    [?php endif;?]</tbody>
</table>
