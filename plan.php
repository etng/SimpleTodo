...
case 'add':
    ...
    if(isHttpPost())
    {
        ...
        $plan_record = array_merge($_POST['plan'], compact('status', 'car_status', 'room_status', 'user_id', 'contact_id', 'created'));
        $plan_id = $db->insert('plan', $plan_record);
...
case 'view':
    ...
    $plan['tourists'] = $db->fetchAll('select
        *
        from tourist
        where id in (select tourist_id from plan_tourist where plan_id=' . $plan['id'] . ')', 'id');
    $plan['notes'] = $db->fetchAll('select
        plan_note.*
        ,staff.name as staff_name
        from plan_note
        left join staff on staff.id=plan_note.staff_id
        where plan_id=' . $plan['id'] . '
        order by plan_note.created desc', 'id');
    $plan['tours'] = $db->fetchAll('select
        *,
        plan_tour.id as id
        from plan_tour
        left join tour on tour.id=plan_tour.tour_id
        where plan_tour.plan_id=' . $plan['id'], 'id');
    $plan['history'] = $db->fetchAll('select
        *
        from plan_history
        where plan_id=' . $plan['id'] . '
        order by created desc', 'id');
    $plan['payments'] = $db->fetchAll('select
        *
        from plan_payment
        where plan_id=' . $plan['id'] . '
        order by created desc', 'id');
...
case 'get-plan-tour_rooms':
    ...
    echo json_encode($db->fetchAll('select
        ...
        from plan_tour_room
        left join hotel on hotel.id=plan_tour_room.hotel_id
        where plan_tour_id='.$plan_tour_id));
    die();
    break;
case 'get-plan-tour_cars':
    ...
    echo json_encode($db->fetchAll('select
        ...
        from plan_tour_car
        left join driver on driver.id=plan_tour_car.driver_id
        where plan_tour_id='.$plan_tour_id));
    die();
    break;
...
case 'get-room-price':
    ...
    echo json_encode($db->fetchRow($sql='select
        *
        from room_daily_price
        where the_date="'.$the_date.'" and hotel_id="'.$hotel_id.'" and room_type="'.$room_type.'"'));
    die();
    break;
